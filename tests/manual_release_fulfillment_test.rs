...
async fn test_manual_release_waitlist_fulfillment() {
    ...
    assert_eq!(wait_count, 0, "Waitlist entry should have been fulfilled after manual release");

    let (active_lease_owner,): (String,) = sqlx::query_as("SELECT owner_id FROM leases WHERE status = 'ACTIVE'")
        .fetch_one(&pool).await.unwrap();
    assert_eq!(active_lease_owner, "waiting-user", "The waiting user should now have the active lease");
}
