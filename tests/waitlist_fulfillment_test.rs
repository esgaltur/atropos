...
async fn test_waitlist_fulfillment_logic() {
    ...
    repo.waitlist_resource(pool_type.clone(), "waiting-user".into(), "t1".into(), 10).await.unwrap();
    ...
    let lease_opt = repo.fulfill_next_waitlist_entry(pool_type.clone()).await.unwrap();

    // 3. Assert: Lease should be created, waitlist entry should be gone
    assert!(lease_opt.is_some(), "Expected a lease to be created for the waitlisted user");
    ...
    let (wait_count_after,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM waitlist_entries")
        .fetch_one(&pool).await.unwrap();
    assert_eq!(wait_count_after, 0, "Waitlist entry should be removed after fulfillment");

    let (lease_count,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM leases WHERE status = 'ACTIVE'")
        .fetch_one(&pool).await.unwrap();
    assert_eq!(lease_count, 1, "There should be 1 active lease now");

    // 4. Edge Case: No waitlist entry
    let res = repo.fulfill_next_waitlist_entry("Non-Existent-Type".to_string()).await.unwrap();
    assert!(res.is_none());
    ...
    let res = repo.fulfill_next_waitlist_entry(pool_type.clone()).await.unwrap();
