...
async fn test_reaper_performance_baseline() {
    ...
    for chunk in resources.chunks(100) {
        let mut qb = sqlx::QueryBuilder::new("INSERT INTO resources (id, pool_id, external_id, status) ");
    ...
    for chunk in resources.chunks(100) {
        let mut qb = sqlx::QueryBuilder::new("INSERT INTO leases (id, resource_id, owner_id, tenant_id, status, expires_at) ");
        qb.push_values(chunk, |mut b, &res_id| {
